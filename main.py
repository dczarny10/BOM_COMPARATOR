...
def mass_check():
    ...
    while True:
        ...
        if event_m == 'OK':
            ...
            products_list = set([i[:-4] if i.endswith('_SAP') else i[:-10] for i in products]) #create a products lists without _SAP or revision
            for p in products_list:
                products_pairs.append([i for i in products if i.startswith(p)]) #create pairs of SAP and PLM product number
            for c, j in enumerate(products_pairs):
                if len(j) < 2: #no pair in loaded data
                    ws1.cell(row=index_not_found, column=1).value = j[0]
                    index_not_found += 1
                else:
                    ws = wb.create_sheet(title=products_list[c])
    return True
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        try:
            ...
            data_t, products_t, parts_t = PLM_BOM_transpose.load_files(files)
            products += products_t
            parts += parts_t
            data.update(data_t)
        ...
        try:
            files = sg.PopupGetFile('Select folder', no_window=True, multiple_files=True,
                                    file_types=(("Excel files", [".xls", ".xlsx"]),))
            data_subassemblies_t, sub_products_t, sub_parts_t = PLM_BOM_transpose.load_files(files)
